# K 线图黑屏和"未连接"完整修复文档（最终版）

## 问题诊断

前端 K 线图黑屏和"未连接"的原因：

1. **后端订阅流确认**：已确认订阅的是 `@kline` K 线流（不是 `@depth` 盘口数据），但需要支持更多间隔
...
**a) 修正订阅为 Binance K 线流（支持多 interval：1m, 5m, 15m, 1h, 4h, 1d）**
```python
...
# 币安 WebSocket 流格式：<symbol>@kline_<interval>
# 例如：btcusdt@kline_1m, btcusdt@kline_5m, btcusdt@kline_15m, btcusdt@kline_1h 等
streams = []
...
# URL 格式：wss://stream.binance.com:443/ws?streams=btcusdt@kline_1m/btcusdt@kline_5m/...
...
if '@kline_' not in stream_name.lower():
    # 如果不是 K 线流，跳过（可能是其他类型的流，如 @depth）
...
async def _process_kline(self, kline_data):
    ...
    解析币安 WebSocket 返回的 K 线数据：
    {
        's': 'BTCUSDT',  # symbol
        'i': '1m',       # interval
        't': 1234567890, # timestamp (ms) - unix_ms
        'o': '50000.0',  # open
        'h': '51000.0',  # high
        'l': '49000.0',  # low
        'c': '50500.0',  # close
...
async def _broadcast_kline(self, kline: Dict[str, Any]):
    ...
    注意：time 是毫秒级时间戳（unix_ms），前端会转换为秒级时间戳（time / 1000）
    """
    kline_data = {
...
**e) Session 管理（async with ClientSession() in 重连循环）**
```python
...
**f) 无限重连（每 5 秒重试，处理 Session closed/timeout/SSL 错误）**
```python
...
**a) 修正 TypeError：在 mounted 中直接 createChart 和 addCandlestickSeries（不是 this.chart.initChart）**
```javascript
...
**b) 设置图表大小（width: '100%', height: '420px'，添加 resizeObserver 自适应页面）**
```javascript
// ========== 关键修复：创建图表实例 ==========
...
**c) 添加 onmessage 处理推送 K 线数据（series.update(bar) 实时刷新，或 setData(bars) 批量）**
```javascript
...
**d) 初始从数据库/API fetch 历史 K 线（时间戳转换为 TradingView 格式）**
```javascript
// 在 MarketDetail.vue 中：
const fetchKlineHistory = async () => {
  ...
  if (res.data && Array.isArray(res.data)) {
    // ========== 关键修复：时间戳转换为 TradingView 格式（秒级时间戳）==========
...
// TradingViewWidget.vue 会自动通过 props.initialData 接收数据
...
- `[DEBUG] 订阅 K 线流: btcusdt@kline_5m`
- `[DEBUG] 订阅 K 线流: btcusdt@kline_15m`
- `✅ [OK] ========== 币安 WebSocket 连接成功 ==========`
- `[DEBUG] 处理K线数据: symbol=BTC/USDT, interval=1m, timestamp=...`
- `[BROADCAST] 推送 K 线数据格式: time=... (ms), open=..., close=..., volume=...`
...
1. **支持多 interval**：`WS_INTERVALS` 更新为 `["1m", "5m", "15m", "1h", "4h", "1d"]`
2. **订阅流确认**：添加日志确认订阅的是 `@kline` K 线流
3. **流验证**：添加验证，跳过非 K 线流（如 `@depth`）
4. **数据解析**：确保正确解析币安 WebSocket 返回的 K 线数据（time: unix_ms, open, high, low, close, volume）
5. **推送格式优化**：推送标准 JSON 格式，确保 time 是整数（毫秒）
6. **Session 管理**：在重连循环内创建 Session，避免 Session closed 错误
7. **无限重连**：每 5 秒重试一次，处理各种错误类型
...
5. **初始加载**：监听 `props.initialData` 变化，自动加载历史 K 线
6. **主题设置**：涨绿色（#0ECB81），跌红色（#F6465D）
...
5. **时间周期监听**：监听 `selectedTimeframe` 变化，重新拉取 K 线数据
...
1. ✅ **支持多 interval**：`WS_INTERVALS` 更新为 `["1m", "5m", "15m", "1h", "4h", "1d"]`
2. ✅ **订阅流确认**：确认订阅的是 `@kline` K 线流（不是 `@depth`），并添加验证
3. ✅ **TypeError 修复**：修正为在 `mounted` 中调用 `initChart()`（不是 `this.chart.initChart()`）
4. ✅ **图表大小自适应**：宽度 100%（自适应容器），高度 420px，添加 ResizeObserver
5. ✅ **数据更新**：确保数据接收后正确更新 CandlestickSeries
6. ✅ **WebSocket 状态**：确保 `onopen` 和 `onmessage` 中更新状态为"已连接"，避免"未连接"显示
7. ✅ **数据格式**：确保推送格式符合前端要求（time: unix_ms, open: float, high: float, low: float, close: float, volume: float）
