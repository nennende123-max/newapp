<template>
      ...
      <div class="scroll-content">
        ...
        <div class="quick-actions">
          ...
          <div 
            ...
            @click="handleButtonClick('service', '/support')"
            @touchstart="activeButton = 'service'"
            @touchend="setTimeout(() => { activeButton = null }, 150)"
          >
            <div class="icon-circle">
              <van-icon name="service-o" />
            </div>
            <span>{{ $t('home_btn.service') }}</span>
          </div>
        </div>
  ...
  const marketList = computed(() => {
    return coinSymbols.map(symbol => {
      ...
      if (ticker) {
        // 格式化价格显示
        const formattedPrice = formatPrice(ticker.price);
        
        // 格式化成交量显示（USDT 成交量）
        const formattedVol = formatVolume(ticker.quoteVolume);
  ...
  const handleButtonClick = (btnType, path) => {
    ...
    if (btnType === 'service') {
      // 客服按钮的逻辑
      showToast('VIP 客服系统连接中...');
      // router.push('/support'); // 以后有了页面再放开
    } else {
      // 充值、提现、理财直接跳转
      router.push(path);
    }
  ...
  /* 选中/激活状态（可选，如果需要更强的反馈） */
  .feature-btn:active .icon-circle {
    background: #363c45;
    border-color: rgba(252, 213, 53, 0.3); /* 点击时出现微弱金边 */
  }

  /* 图标大小 */
  .icon-circle .van-icon {
    font-size: 26px; /* 图标放大，提升识别度 */
    font-weight: 600;
  }

  /* 图标微调 */
  .rotate-icon {
    transform: rotate(180deg); /* 让箭头向下，代表充值 */
  }

  /* 文字标签 */
  .action-btn span {
    font-size: 13px; /* 字体微调 */
    color: #8E8E93; /* 次要文字颜色，避免抢眼 */
    font-weight: 500;
    letter-spacing: 0.3px;
  }
